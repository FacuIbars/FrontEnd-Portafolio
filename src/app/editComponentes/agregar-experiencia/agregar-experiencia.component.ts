...
export class AgregarExperienciaComponent implements OnInit {
  ...
  id: number | undefined;
  constructor(private fb:FormBuilder,private AppService:ExperienciaService,
...
getExperiencia(id: number) {
  this.AppService.buscarExperiencia(id).subscribe(data => {
    ...
    if(this.id == undefined){
      //Agregar
      this.AppService.crearExperiencia(Experiencia).subscribe(()=>{this.loading= false; 
        
        
      } )
    }else{
      //Editar
      this.AppService.updateExperiencia(Experiencia, this.id).subscribe(data => {
